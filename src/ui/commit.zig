...
fn showCommitDiff(ctx: *gitweb.Context, repo: *git.Repository, commit_obj: *git.Commit, writer: anytype) !void {
    ...
    // Create diff
    var diff = try git.Diff.treeToTree(
        repo.repo,
        if (parent_tree) |pt| pt.tree else null,
        commit_tree.tree,
        null
    );
    defer diff.free();
    ...
    try diff.print(
        c.GIT_DIFF_FORMAT_PATCH,
        @TypeOf(callback_data).printLine,
        @constCast(@ptrCast(&callback_data))
    );
