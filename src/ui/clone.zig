        ...
        if (head_oid != null) {
            ...
            if (first) {
                const line = try std.fmt.allocPrint(ctx.allocator, 
                    "{s} HEAD\x00multi_ack thin-pack side-band side-band-64k ofs-delta shallow deepen-since deepen-not deepen-relative no-progress include-tag multi_ack_detailed symref=HEAD:refs/heads/master agent=zgit/0.1\n",
                    .{oid_str}
                );
        ...
        if (target == null) {
            ...
            if (c.git_reference_resolve(&resolved, ref) == 0) {
                ...
                if (resolved_target != null) {
                    ...
                    if (first) {
                        const line = try std.fmt.allocPrint(ctx.allocator,
                            "{s} {s}\x00multi_ack thin-pack side-band side-band-64k ofs-delta shallow deepen-since deepen-not deepen-relative no-progress include-tag multi_ack_detailed agent=zgit/0.1\n",
                            .{ oid_str, name }
                        );
                        defer ctx.allocator.free(line);
                        try writePktLine(writer, line);
